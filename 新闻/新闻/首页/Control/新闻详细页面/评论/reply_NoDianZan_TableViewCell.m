...
//    m_name.text = model.myUserModel.user_name;
    if(model.ToUserModel.user_name.length > 0){ //对他人的回复
        NSString* str_all = @"";
        if(![model.ToUserModel.user_id isEqualToString:self.main_model.myUserModel.user_id]){
            str_all = [NSString stringWithFormat:@"%@ 回复 %@:",model.myUserModel.user_name,model.ToUserModel.user_name];
        }
        else{
            str_all = [NSString stringWithFormat:@"%@ 回复:",model.myUserModel.user_name];
        }
